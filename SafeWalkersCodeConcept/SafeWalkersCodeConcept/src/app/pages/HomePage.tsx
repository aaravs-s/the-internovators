      ...
      <div className="relative shrink-0 w-full">
        ...
        <div className="flex items-center justify-between pb-[17px] pt-[28px] px-[32px]">
          <div>
            ...
            <p className="font-['Inter',sans-serif] font-normal text-[14px] text-[rgba(255,255,255,0.4)]">Stay safe on every route.</p>
          </div>
      ...
      <div className="px-[32px] py-[24px] flex flex-col gap-[28px]">
        ...
        <div className="flex gap-[16px]">
          ...
          <div className="flex-1 rounded-[20px] p-[24px] relative overflow-hidden" style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)" }}>
            <div className="flex items-start justify-between mb-[16px]">
              <div>
                <p className="font-['Inter',sans-serif] font-medium text-[11px] text-[rgba(255,255,255,0.4)] uppercase tracking-[0.8px] mb-[4px]">Area Safety Index</p>
              ...
              <div className="size-[52px] rounded-[14px] flex items-center justify-center" style={{ background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.2)" }}>
                <div className="size-[10px] rounded-full bg-[#22c55e]" />
              </div>
            </div>
          ...
          <div className="flex flex-col gap-[10px]">
            {[
              ...
              <div key={s.label} className={`${cardBase} px-[18px] py-[13px] flex-1`} style={{ minWidth: "230px" }}>
                <p className="font-['Inter',sans-serif] font-normal text-[10px] text-[rgba(255,255,255,0.35)] uppercase tracking-[0.6px]">{s.label}</p>
                <p className="font-['Inter',sans-serif] font-bold text-[22px] text-white tracking-[-0.5px] leading-tight">{s.value}</p>
                <p className="font-['Inter',sans-serif] font-normal text-[11px] text-[rgba(255,255,255,0.35)] mt-[2px]">{s.sub}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`${cardBase} p-[24px]`}>
          <div className="flex items-center justify-between mb-[16px]">
            ...
            <div className="flex gap-[8px]">
              {["km", "routes"].map((t, i) => (
                <button key={t} className={`px-[12px] py-[5px] rounded-[10px] font-['Inter',sans-serif] font-medium text-[12px] cursor-pointer ${i === 0 ? "bg-[rgba(196,32,80,0.2)] text-[#c42050] border border-[rgba(196,32,80,0.3)]" : "text-[rgba(255,255,255,0.4)]"}`}>{t}</button>
              ))}
            </div>
          </div>
          <ResponsiveContainer width="100%" height={100}>
            <BarChart data={weekActivity} barCategoryGap="30%">
              ...
              <Tooltip contentStyle={{ background: "rgba(20,10,15,0.95)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", color: "white", fontSize: "12px" }} cursor={{ fill: "rgba(255,255,255,0.04)" }} />
              <Bar dataKey="km" radius={[5, 5, 0, 0]}>
                {weekActivity.map((_, i) => (
                  <Cell key={i} fill={i === 5 ? "url(#barGrad)" : "rgba(255,255,255,0.1)"} />
                ))}
              </Bar>
              <defs>
        ...
        <div>
          <div className="flex items-center justify-between mb-[16px]">
            ...
            <button onClick={() => navigate("/saved")} className="font-['Inter',sans-serif] font-medium text-[13px] text-[#0a84ff] cursor-pointer">See all</button>
          </div>
        ...
        <div>
          ...
          <button onClick={() => navigate("/profile")} className={`${cardBase} w-full text-left cursor-pointer hover:border-[rgba(255,255,255,0.15)] transition-colors`}>
