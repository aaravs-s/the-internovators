...
export const cardBase = "bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.08)] rounded-[20px]";
    ...
    <div className="absolute flex flex-col h-[944px] items-center justify-center left-0 px-[64px] top-0 w-[774.5px]">
      <div className="flex flex-col gap-[40px] items-center">
        ...
        <div className="w-[282px] flex flex-col items-center">
          ...
          <p className="font-['Inter',sans-serif] font-normal text-[16px] text-[rgba(255,255,255,0.4)] text-center leading-[26px] pt-[16px]">
...
export function PrimaryButton({ label, onClick, wide }: { label: string; onClick?: () => void; wide?: boolean }) {
  return (
    <button
      onClick={onClick}
      type="submit"
      className={`drop-shadow-[0px_6px_7px_rgba(160,24,64,0.5)] flex gap-[8px] h-[52px] items-center justify-center relative rounded-[16px] shrink-0 cursor-pointer transition-opacity hover:opacity-90 active:opacity-80 ${wide ? "w-full" : "w-[384px]"}`}
...
export function FormInput({ label, placeholder, type = "text", value, onChange }: {
  ...
  return (
    <div className="relative w-full">
      <p className="font-['Inter',sans-serif] font-semibold text-[13px] text-[rgba(255,255,255,0.85)] tracking-[0.065px] mb-[8px]">{label}</p>
      <div className="bg-[rgba(255,255,255,0.07)] h-[50px] relative rounded-[14px] w-full">
        ...
        <input
          ...
          className="absolute inset-0 bg-transparent px-[17px] text-[15px] font-['Inter',sans-serif] text-white placeholder-[rgba(255,255,255,0.28)] tracking-[-0.15px] outline-none rounded-[14px]"
...
export function SafetyBadge({ score }: { score: number }) {
  ...
  const color  = normalizedScore >= 9 ? "#22c55e" : normalizedScore >= 7.5 ? "#f59e0b" : "#ef4444";
...
export function Tabs({ tabs, active, onChange }: { tabs: string[]; active: string; onChange: (t: string) => void }) {
  return (
    <div className="flex gap-[2px] bg-[rgba(255,255,255,0.05)] rounded-[14px] p-[3px]">
      {tabs.map((t) => (
        <button key={t} onClick={() => onChange(t)}
          className={`px-[16px] py-[7px] rounded-[11px] cursor-pointer font-['Inter',sans-serif] font-medium text-[13px] tracking-[-0.1px] transition-colors ${active === t ? "bg-[rgba(196,32,80,0.2)] text-[#c42050] border border-[rgba(196,32,80,0.3)]" : "text-[rgba(255,255,255,0.4)]"}`}>
          {t}
        </button>
      ))}
    </div>
...
export function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    <button onClick={onToggle}
      ...
      style={on ? primaryGradient : { background: "rgba(255,255,255,0.12)" }}
    >
      ...
      {[1,2,3,4,5].map((s) => (
        <svg key={s} width="12" height="12" viewBox="0 0 12 12" fill="none">
          <path d="M6 1L7.545 4.13L11 4.635L8.5 7.07L9.09 10.51L6 8.885L2.91 10.51L3.5 7.07L1 4.635L4.455 4.13L6 1Z"
            fill={s <= value ? "#c42050" : "rgba(255,255,255,0.15)"} />
        </svg>
      ))}
    </div>
  );
...
export function RouteCard({ distance, duration, label, safety, onClick }: {
  ...
  return (
    <div onClick={onClick} className="bg-[rgba(255,255,255,0.05)] rounded-[20px] w-[340px] flex-shrink-0 overflow-hidden relative border border-[rgba(255,255,255,0.08)] cursor-pointer hover:border-[rgba(255,255,255,0.16)] transition-colors">
        ...
        <div>
          ...
          <div className="flex gap-[10px] items-center">
            <div className="flex gap-[4px] items-center">
              ...
              <span className="font-['Inter',sans-serif] font-normal text-[12px] text-[rgba(255,255,255,0.6)]">{distance}</span>
            </div>
            <div className="bg-[rgba(255,255,255,0.1)] h-[10px] w-px" />
            <span className="font-['Inter',sans-serif] font-normal text-[12px] text-[rgba(255,255,255,0.6)]">{duration}</span>
          </div>
        </div>
...
export function IconBell({ color, badge }: { color: string; badge?: number }) {
  return (
    <div className="relative">
      ...
      {badge && badge > 0 ? (
        <div className="absolute -top-[4px] -right-[4px] size-[14px] rounded-full bg-[#c42050] flex items-center justify-center">
