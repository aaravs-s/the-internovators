...
export default function ProfilePage() {
  ...
  return (
    <>
      <header className="flex items-center justify-between border-b border-[var(--section-divide-border)] px-[32px] pb-[17px] pt-[28px]">
        <div><h1 className="text-[38px] font-bold tracking-[-0.8px] text-white">My Profile</h1><p className="text-[14px] text-[var(--text-note-subtitle)]">Your public Walking Community presence.</p></div>
        <button onClick={() => navigate("/settings")} className="flex h-[40px] items-center gap-[8px] rounded-[12px] border border-[var(--select-border)] bg-[var(--card-background-secondary)] px-[14px] text-[13px] text-[var(--text-body)]"><IconGear color="var(--small-text-grey)" />Settings</button>
      </header>
      <main className="max-w-[900px] p-[32px] flex flex-col gap-[18px]">
        {loading && <p className="text-[13px] text-[var(--text-note-subtitle)]">Loading your community profile…</p>}
          ...
          <>
            <section className={`${cardBase} p-[24px]`}>
              <div className="flex items-center gap-[18px]">
                ...
                <button onClick={() => editing ? void saveBio() : setEditing(true)} className="rounded-[12px] border border-[var(--grey-light-border-hover)] bg-[var(--section-divide-border)] px-[16px] py-[8px] text-[13px] font-semibold text-[var(--text-body)]">{editing ? "Save" : "Edit Profile"}</button>
              </div>
              {editing && <textarea aria-label="Profile bio" value={bio} onChange={(event) => setBio(event.target.value)} rows={2} className="mt-[16px] w-full resize-none rounded-[10px] border border-[var(--select-border)] bg-[var(--option-bg-hover)] px-[12px] py-[8px] text-[14px] leading-[22px] text-[var(--text-body)] outline-none" />}
            </section>
